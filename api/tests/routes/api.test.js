...
describe('Test API routes', () => {
  describe('Test "/api/foos" path', () => {
    test('Should respond an array of objects with a 200', async () => {
      const response = await request(app).get('/api/foos').authenticate();
  ...
  describe('Test "/api/foos/add" path', () => {
    test('Should respond an object with a 201', async () => {
      const response = await request(app).post('/api/foos/add').send({ name: 'foo' }).authenticate();
  ...
  describe('Test "/api/foos/:id" path', () => {
    test('Should respond with a 500 with a malformed id', async () => {
      const response = await request(app).get('/api/foos/42').authenticate();
      expect(response.statusCode).toBe(500);
    });

    test('Should respond with a 200 with valid id but foo does not exist', async () => {
      const response = await request(app).get('/api/foos/5bfe0af364e3bf0830f649ba').authenticate();
      expect(response.statusCode).toBe(200);
    ...
    test('Should respond with a 200 with an existing foo', async () => {
      // add foo
      let response = await request(app).post('/api/foos/add').send({ name: 'bar' }).authenticate();
      const foo = response.body;
      response = await request(app).get(`/api/foos/${foo._id}`).authenticate();
      expect(response.statusCode).toBe(200);
  ...
  describe('Test "/api/foos/test" path', () => {
    test('Should respond an object with a 200', async () => {
      const response = await request(app).get('/api/foos/test').authenticate();
      expect(response.statusCode).toBe(200);
